...
```{r Environment}
library(tidyverse)
setwd('~/Documents/Projects/Methanogens/')
...
compound_data <- read_csv('./Data/FTICR/MetaboDirect_Out_Binary/1_preprocessing_output/Report_processed_MolecFormulas.csv') %>%
...
hilic_bad <- read_lines('Data/R_Prepped/HILIC_Duplicate_Features.txt')
hilic_IS_frags <- read_lines('Data_cleaning/IS_Remove_HILIC.txt')
hilic_badFormulas <- read_lines('Data_cleaning/hilic_badFormula.txt')

#Make the data ready for analysis
hilic_raw <- read_csv('./Data/R_Prepped/HILIC_Data_wQCs.csv') %>%
...
#Load in duplicate features which need to be removed from the data
rp_bad <- read_lines('Data/R_Prepped/RP_Duplicate_Features.txt')
rp_IS_frags <- read_lines('Data_cleaning/IS_Remove_RP.txt')
rp_badFormulas <- read_lines('Data_cleaning/rp_badFormula.txt')

#Load in the data
rp_raw <- read_csv('./Data/R_Prepped/RP_Data.csv') %>%
  #Remove norm area from file names
  rename_with(~gsub('Norm. Area: ', '', .x)) %>%
  #Remove .raw from file names
  rename_with(~gsub('.raw', '', .x)) %>%
...
#Load in metadata
meta_hilic <- read_csv('./Data/R_Prepped/metadata_HILIC.csv') %>%
  #Fix the day type for fen
  mutate(Day = ifelse(Site == 'fen' & Day == 'D28', 'D35', Day))

meta_rp <- read_csv('./Data/R_Prepped/metadata_RP.csv') %>%
  #Fix the day type for fen
  mutate(Day = ifelse(Site == 'fen' & Day == 'D28', 'D35', Day))
...
canopus_rp <- read_csv('Data/R_Prepped/Canopus_RP_Consensus.csv') %>%
  modify_at('progenesis_id', ~paste0('FT_', .x))

canopus_hilic <- read_csv('Data/R_Prepped/Canopus_HILIC_Consensus.csv') %>%
  modify_at('progenesis_id', ~paste0('FT_', .x))

link_rp <- read_csv('Data/R_Prepped/CompoundLink_RP.csv')  %>%
...
link_hilic <- read_csv('Data/R_Prepped/CompoundLink_HILIC_Fix.csv') %>%
  modify_at('Compound_ID', ~paste0('FT_', .x)) %>%
  mutate(Name = ifelse(abs(`Annot. DeltaMass [ppm]`) > 5, NA, Name)) %>%
...
source('~/Documents/Projects/Scripts/ChemRichFunctions.R')
...
huh <- c('#616e91', '#3333FF', '#009900')
names(huh) <- c('None', 'Bog', 'Fen')
dummy <- data.frame(site = c('None', 'Bog', 'Fen'),
                    color = huh)
plotChemRich_custom(cr_input, colors = c('#3333FF', '#009900'), sets = plot_set, binary = T) +
  cowplot::theme_cowplot() +
...
fen_day <- c('#5DC0FE', '#018ADF', '#024AF2', '#01267A')
names(fen_day) <- c('fen_D0', 'fen_D7', 'fen_D14', 'fen_D35')
bog_day <- c('#6FFF47', '#3BDE00', '#0F8D01', '#276501')
...
cr_plot <- plotChemRich_custom(cr_input, colors = c('#3333FF', '#009900'), sets = plot_set, binary = T) +
  cowplot::theme_cowplot()  +
  scale_x_continuous(labels = NULL) +
...
pmo <- readxl::read_excel('./FinalFinalScripts/yanni_gene_curation.xlsx')
...
nar <- readxl::read_excel('./FinalFinalScripts/yanni_gene_curation.xlsx', sheet = 2)
...
auto_scale <- function(x){
  mtb_scaled <- as.data.frame(apply(x, 2, AS_helper))
  return(mtb_scaled)
}
...
NS_ranged <- tea_tc_fen_ranged %>%
  filter(tea %in% c('NR_all', 'SR_all', 'sox_all', 'nox_all')) %>%
  modify_at('tea', factor, levels = c('NR_all', 'SR_all', 'nox_all', 'sox_all'))

ns_plot_r <- ggplot(NS_ranged, aes(x = day, y = value, color = tea, group = tea, fill = tea)) +
  geom_smooth(linewidth = 2, se = F) +
   scale_color_manual(values = tea_pal, labels = c('Nitrate Reduction', 'Sulfate Reduction', 
                                                   'Nitrification', 'Sulfur Oxidation'),
                      name = 'Process') +
...
a_leg <- cowplot::get_legend(ns_plot_r)
ns_plot_r_nl <- ns_plot_r +
  theme(legend.position = 'none')
...
c_plot_a <- ggplot(C_actual, aes(x = day, y = value, color = tea, group = tea, fill = tea)) +
  geom_smooth(linewidth = 2, se = T) +
...
c_ranged <- tea_tc_fen_ranged %>%
  filter(tea %in% c('ox_all', 'ferm_all', 'hemi_all', 'methane_all', 'methox_all')) %>%
  modify_at('tea', factor, levels = c('ox_all', 'ferm_all', 'hemi_all', 'methane_all', 'methox_all'))

c_plot_r <- ggplot(c_ranged, aes(x = day, y = value, color = tea, group = tea, fill = tea)) +
  geom_smooth(linewidth = 2, se = F) +
...
b_leg <- cowplot::get_legend(c_plot_r)
c_plot_r_nl <- c_plot_r +
  theme(legend.position = 'none')


cowplot::plot_grid(ns_plot_a, ns_plot_r_nl, a_leg, c_plot_a, c_plot_r_nl, b_leg,
...
db_carbon <- readxl::read_excel('~/Downloads/metabolism_summary.xlsx',
...
db_all <- read_delim('~/Downloads/annotations.tsv')
...
cowplot::plot_grid(g_plot3, m_plot, t_plot3, nrow = 1, axis = 'tblr', align = 'h',
...
gas <- read_csv('./Data/R_Prepped/gas_data.csv')
...
day_pal_cor <- c('#5DC0FE', '#018ADF', '#0255B7', '#0238B6', '#01267A')
...
cowplot::plot_grid(gplot, thr_panel, egm_plot, ph_panel, splot,  bf_panel, ncol = 2,
                   rel_widths = c(7,10), labels = c('A', 'D', '', 'E', 'C', 'F'))
