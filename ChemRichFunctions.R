...
plotChemRich_custom <- function(CR_output, interactive = FALSE, colors = c('blue', 'red'), sets, 
                                text_size = 4, binary = F, sig_line = 0.05){
  if(binary){
    CR_output <- CR_output %>%
      #filter(uprat >= 0.65 | uprat <= 0.45) %>%
      mutate(uprat = ifelse(uprat >= 0.65, 1, 
                            ifelse(uprat <= 0.45, 0, 0.5)))
  }
  p <- suppressMW(ggplot(CR_output, aes(x = order, y = -log(ptest), size = size, color = uprat)) +
                    ...
                    scale_color_gradient(low = colors[1], high = colors[2], limits = c(0,1)) +
                    scale_size(range = c(5,30)) +
                    ggrepel::geom_label_repel(aes(label = set), color = 'gray20', data = subset(CR_output, set %in% sets), size = text_size, force = 5) +
                    theme(legend.position = 'none')) 
  if(interactive){
    suppressMW(plotly::ggplotly(p, tooltip = 'text'))
  }else{
